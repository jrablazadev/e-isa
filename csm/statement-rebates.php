<!doctype html>
<html lang="en">
	<head>
		<?php include_once "../partials/head.php"; ?> 
		<title>E-ISA</title>
	</head>
	<body class="hide">

		<?php include_once "../partials/navbar-csm.php"; ?>
		
		<main role="main" class="container">

			<div class="ishidden">
				<!-- navigation bar parameters container -->
				<script type="text/javascript" src="https://caspio.thenetwork-crm.com/dp/25d55000288517adb7084773b2e2/emb"></script>
			</div>
			
			<div style="ishidden">
				<script type="text/javascript" src="https://caspio.thenetwork-crm.com/dp/25d55000b8fc270763644165bbcd/emb"></script>
			</div>
		
            		<?php include_once "../partials/progressbar.php"; ?>

			<div class="cb-statement-rebate-bulk-update">
				<script type="text/javascript" src="https://caspio.thenetwork-crm.com/dp/25d550008ad03d638fd340368fc2/emb"></script>
			</div>

			<hr>

			<div class="marginbot20px section-container container1 grid-edit cb-statement-rebate">
				<!-- Section 5 : ISA Statement Rebates -->
				<script type="text/javascript" src="https://caspio.thenetwork-crm.com/dp/25d550000acbae05d7954542a1cd/emb"></script>
			</div>
			
			<div class="marginbot20px calc-summary-container">
				<script type="text/javascript" src="https://caspio.thenetwork-crm.com/dp/25d55000e6fa8ea67191406f98e4/emb"></script>
			</div>
			
			<div class="marginbot20px">
				<!-- COMMENTS SECTION -->
				<script type="text/javascript" src="https://caspio.thenetwork-crm.com/dp/25d55000f80be512f9f14c39ba69/emb"></script>
			</div>

		</main>

		<style>
			.cbResultSetGridNonEditableColumnNumberDate .Data,
			.Data,
			input[name^="InlineEdit"]
			{
				text-align:left !important;
			}

			.cbResultSetGridHeaderCellNumberDate
			{
				text-transform:none !important;
			}

			/* .container1 tr[data-cb-name="data"] > td:nth-child(2n+2)
			{
				background-color:lightblue !important;
			}

			.container1 tr[data-cb-name="data"] > td:nth-child(5)
			{
				background-color:lightblue !important;
			}

			.container1 tr[data-cb-name="data"] > td:nth-child(1n+7)
			{
				background-color:white !important;
			} */
		</style>
		<script>
			// function percent_columns()
			// {
			// 	$('.statement').each(function(){
			// 		var val = Number(this.value);
			// 		$(this).closest('tr').children('td:nth-child(4)').html(val.toFixed(2).toString() + '%');
			// 	});
			// }

			// document.addEventListener('DataPageReady', function (event) {
			// 	percent_columns();
			// });
		</script>
	</body>
</html>