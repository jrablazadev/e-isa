document.addEventListener('DataPageReady', function (event) {
	...
	if(event.detail.appKey == '25d55000c736d9fd7ac94c7ba720') 
	{
		...
		initMultiSelect('*[name="cbParamVirtual7"]', '*[name="cbParamVirtual9"]');
		initMultiSelect('*[name="cbParamVirtual8"]', '*[name="cbParamVirtual10"]');

		$('.dropdown.bootstrap-select.show-tick').addClass('p-0');
		$('*[name="cbParamVirtual7"], *[name="cbParamVirtual8"]').closest('div').find('.btn.dropdown-toggle').css({
			'font-size' : '14px',
			'padding' : '3px 6px'
		});
...
function highlight_doc_exeption_report()
{
	$('#cb-doc-exep-report tr[data-cb-name="data"]').each(function(){
		...
		{
			$(this).children('td').eq(15).addClass('cb-red font-weight-bold alert-warning'); // LB Attachment Count
		}

		if( requested_credit_limit > 0 && credit_app_attachments == 0 )
		{
			$(this).children('td').eq(16).addClass('cb-red font-weight-bold alert-warning'); // Credit App Attachment Count
		}

		if( upfront_cash > 0 && w9_attachments == 0 )
		{
			$(this).children('td').eq(17).addClass('cb-red font-weight-bold alert-warning'); // W9 Attachment Count
		}
		...
		{
			$(this).children('td').eq(18).addClass('cb-red font-weight-bold alert-warning'); // Signed LOU Attachment Count
		}
	});
}
