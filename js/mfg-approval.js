document.addEventListener('DataPageReady', function (event) {
        ...
        if(event.detail.appKey == '25D55000751158e2bf5d47c8916a')
        {
                ...
                $('.cb-attch-vendor-approval .cbResultSetRecordMessage').html('No attachments.');
        }
});
...
{
        var ISAID = document.querySelector('.cb-attch-vendor-approval').getAttribute('data-isaid');

        var scriptElem = document.createElement('script');
        scriptElem.src = 'https://caspio.thenetwork-crm.com/dp/25D55000751158e2bf5d47c8916a/emb?ISA_ID=' + ISAID;

        document.querySelector('.cb-attch-vendor-approval').appendChild(scriptElem);
}
...
function deploy_paintlines()
{
        ...
        var scriptElem = document.createElement('script');
        scriptElem.src = 'https://caspio.thenetwork-crm.com/dp/25D55000c40a9c6e6bee4795993f/emb?ISA_ID=' + ISAID;
...
function deny(deal_approver, comment)
{
        ...
        $('*[name$="RecordNext_Approver_ID"]').val( $('*[name$="Last_Approver_ID"]').val() );
        $('*[name$="Last_Approver_ID"]').val(0);
        ...
        $.confirm({
                ...
                buttons : {
                        ...
                        OK :
                        {
                                ...
                                action: function()
                                {
                                        var mfr_name = $('.cb-popup-mfr-name').val().trim();
        ...
        $.confirm({
                ...
                buttons : {
                        ...
                        OK :
                        {
                                ...
                                {
                                        var mfr_name = $('.cb-popup-mfr-name').val().trim();
                                        var mfr_comment = $('.cb-popup-mfr-comment').val().trim();

                                        if(mfr_name == '') {$('.req-mfr-name-text').show(); return false; } 
                                        if(mfr_comment == '') {$('.req-mfr-comment-text').show(); return false; } 

                                        if(mfr_comment != '' && mfr_name != '')
                                        {
