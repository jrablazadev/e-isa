...
function execute_isa(ISA_ID) {
	jqueryConfirm_ExecuteISA = $.confirm({
		...
		title: 'Execute ISA',
		theme: 'material',
		content: '<iframe id="execute-isa-iframe" src="./execute-isa.php?ISA_ID=' + ISA_ID + '" style="height:120px;"></iframe>',
		...
		buttons: {
			...
			Yes_Btn: {
				...
				action: function() {
					...
					$('#execute-isa-iframe').contents().find('.cb-spinner').html(' <div class="spinner-border text-primary" role="status"></div> <div>Saving please wait..</div> ');
					$('#execute-isa-iframe').contents().find('.cb-dp-container').hide();

					$('#execute-isa-iframe').contents().find('*[name="Mod0EditRecord"]').click();
	...
	$.confirm({
		...
		buttons: {
			...
			YES: {
				...
				btnClass: 'btn-green',
				action: function() {
					window.location.href = './scratch-pad-delete.php?User_Group=Y&ISA_ID=' + ISA_ID;
				}
			}
		}
	});
...
function exect_work_queue_view(isa_id) {
	...
	jqueryConfirm_ExecuteISA = $.confirm({
		...
		theme: 'material',
		content: '<iframe id="cb-exectqueue-view-iframe" src="./dept-execution-status.php?ISA_ID=' + isa_id + '" style="height:70vh;"></iframe>',
		backgroundDismiss: false,
		buttons: false,
		boxWidth: '70%',
		useBootstrap: false,
		onClose: function() {
			if (refresh) {
				$('.cb-execution-work .cbSearchButton').eq(0).click();
			}
		}
	});
}
...
function return_to_contact_phase(isa_id) {
	...
	jQueryConfirm_ReturnToContractPhase = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-return-to-contract-phase', 'https://caspio.thenetwork-crm.com/dp/25D55000694e8349e835424b8c23/emb?ISA_ID=' + isa_id);
		}
	});
}