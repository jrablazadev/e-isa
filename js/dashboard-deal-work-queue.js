const dashboard_dealWorkQueue = {
	...
	percent_complete_graph : function (elem) {
		...
		const leftProgressBar = '<span class="progress-bar border-primary"></span>';
		const progressRight = '<span class="progress-right">';
		const rightProgressBar = '<span class="progress-bar border-primary"></span>';
		const progressValue = '<div class="progress-value w-100 h-100 rounded-circle d-flex align-items-center justify-content-center">';
		...
		$(elem).each(function () {
			...
			$(this).attr('onclick', 'dashboard_dealWorkQueue.open_mpo_detail_report(this)');
			$(this).addClass('cb-mpo-completion')

			if (percentage > 0) {
				if (percentage <= 49.9) {
					if (percentage >= 0 && percentage <= 9.9) {
						return right.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(0) + 'deg)');
					}

					if (percentage >= 10 && percentage <= 19.9) {
						return right.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(10) + 'deg)');
					}

					if (percentage >= 20 && percentage <= 29.9) {
						return right.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(20) + 'deg)');
					}

					if (percentage >= 30 && percentage <= 39.9) {
						return right.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(30) + 'deg)');
					}

					if (percentage >= 40 && percentage <= 49.9) {
						return right.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(40) + 'deg)');
					}
				}
					...
					if (percentage >= 50 && percentage <= 59.9) {
						return left.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(50 - 50) + 'deg)');
					}

					if (percentage >= 60 && percentage <= 69.9) {
						return left.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(60 - 50) + 'deg)');
					}

					if (percentage >= 70 && percentage <= 79.9) {
						return left.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(70 - 50) + 'deg)');
					}

					if (percentage >= 80 && percentage <= 89.9) {
						return left.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(80 - 50) + 'deg)');
					}

					if (percentage >= 90 && percentage <= 99.9) {
						return left.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(90 - 50) + 'deg)');
					}

					if (percentage >= 100) {
						return left.css('transform', 'rotate(' + dashboard_dealWorkQueue.percentageToDegrees(100 - 50) + 'deg)');
					}
				}
			}
	...
	dataPageReady : function() {
		document.addEventListener('DataPageReady', function (event) {
			dashboard_dealWorkQueue.dataPage_fields.map((i) => {
				if ((i).toLowerCase().contains((event.detail.appKey).toLowerCase())) {
					dashboard_dealWorkQueue.percent_complete_graph(i);
				}
			});
		});
	},
