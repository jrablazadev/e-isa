...
document.addEventListener("DataPageReady", function (event) {
	if (event.detail.appKey == "25d5500051ba494d753f49b0a82e") {
		$(
			".qty, .participation, .mfr-participation, .customer-participation"
		).each(function () {
			autonumericid++;

			$(this).addClass("autonumeric");
			$(this).attr({ id: "autonumeric-" + autonumericid });
			$("#autonumeric-" + autonumericid).autoNumeric("init", {
				mDec: 10,
				vMin: 0,
				aSep: "",
				aPad: false,
...
function get_values() {
	...
	$(".qty").each(function () {
		investment_id +=
			$(this).attr("data-investmentid").padStart(10, "0") +
			"|";
	});
	$(".qty").each(function () {
		qty += numeric_padding(this.value) + "|";
	});
	$(".participation").each(function () {
		participation += numeric_padding(this.value) + "|";
	});
	$(".mfr-participation").each(function () {
		mfr_participation += numeric_padding(this.value) + "|";
	});
	$(".customer-participation").each(function () {
		custom_part += numeric_padding(this.value) + "|";
	});
...
function info_popup(id, title) {
	$.confirm({
		...
		theme: "material",
		content:
			'<iframe src="./investments-info.php?ID=' +
			id +
...
function comments_popup(id, title) {
	$.confirm({
		...
		theme: "material",
		content:
			'<iframe src="./investments-notes.php?ID=' +
			id +
			'" onload="iframe_autoresize(this)"></iframe>',
		backgroundDismiss: true,
		buttons: false,
...
function validate_qty() {
	...
	$(".qty").each(function () {
		...
		var title = (
			row.find(".investmentname").attr("data-name") || ""
		)
			.toString()
			.trim();
		...
		switch (investment_id) {
			...
			case 18:
			case 44:
				var participation = Number(
					row.find(".participation").val()
				);
				var mfr_participation = Number(
					row.find(".mfr-participation").val()
				);
