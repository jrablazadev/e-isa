...
function relate_all_accounts_popup(isaId) {
	...
	relateAllAccountsPopup = $.confirm({
		...
		content:
			...
			<div><button type="button" class="btn btn-primary btn-sm mb-5 d-none cb-relate-all-accounts-submit-btn" onclick="relate_all_accounts_submit(' +
			isaId +
			')">Submit</button> </div> \
		...
		onOpen: function () {
			deployDP(
				...
				"https://" +
					...
					"/dp/" +
					appKeyPrefix +
					"c36bdfdc8896457e8efa/emb?ISA_ID=" +
					isaId
			);
			deployDP(
				"cb-relate-all-accounts-add",
				"https://" +
					accountId +
					"/dp/" +
					appKeyPrefix +
					"d424fe7ab6ae4af9896a/emb?ISA_ID=" +
					isaId
			);
		},
	});
		...
		onOpen: function () {
			deployDP(
				...
				"https://" +
					...
					"/dp/" +
					appKeyPrefix +
					"3e3ba698601043499ed2/emb?ISA_ID=" +
					isaId
			);
		},
	});
...
function relate_all_accounts_submit(isaId) {
	$("#cb-relate-all-accounts-submit").html(
		'<iframe src="../shared/add-related-accounts.php?ISA_ID=' +
			isaId +
			"&Network_Customer_IDs=" +
			selectedAccountsRid +
			'"></iframe>'
	);
	...
	$("#cb-relate-all-accounts-main").append(
	...
	deleteRelatedAccountsPopup = $.confirm({
		...
		buttons: {
			...
			buttonNo: {
				...
				action: function () {
					$(
						".cb-delete-account-related-spinner"
					).append(
						'<div class="spinner-border text-info" role="status"></div>'
					);
		...
		onOpen: function () {
			...
			$("#cb-delete-related-account-cn").html(
				'<iframe id="cb-delete-relate-account-fr" src="../shared/delete-related-accounts.php?ID=' +
					relatedAccountId +
					"&ISA_ID=" +
					isaId +
					'"></iframe>'
...
$(document).on(
	"click",
	"#cb-relate-all-accounts-add .cb-account-checkbox",
	function () {
		if (this.checked) {
			if (selectedAccountsRidArr.indexOf(this.value) !== -1) {
				// check account if already exist
				// don't store
			} else {
				selectedAccountsRidArr.push(this.value);
			}
		} else {
			selectedAccountsRidArr.splice(
				selectedAccountsRidArr.indexOf(this.value),
				1
			);
		}
		...
		if (selectedAccountsRid == "") {
			$(".cb-relate-all-accounts-submit-btn").addClass(
				"d-none"
			);
		} else {
			$(".cb-relate-all-accounts-submit-btn").removeClass(
				"d-none"
			);
		}
	}
...
document.addEventListener("DataPageReady", function (event) {
	...
	if (
		...
		"25d55000d424fe7ab6ae4af9896a".toUpperCase()
	) {
		// add related accounts
		$.each(selectedAccountsRidArr, function (key, value) {
			$('.cb-account-checkbox[value="' + value + '"]').prop(
				"checked",
				true
			);
		});
