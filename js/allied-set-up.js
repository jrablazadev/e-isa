...
$(document).on('click', '.invoice-discount tr[data-cb-name="data"] td:nth-child(3)', function(){
	...
	price_example_popup(price_ex, manufacturer, product_line, invoice_discount, price_level);
});

document.addEventListener('DataPageReady', function (event) {
	if((event.detail.appKey).toUpperCase() == ('25d55000596f88aa63cf4078b538').toUpperCase()) // Re-submit to execution
	{
		$('#cb-resubmit-to-exec-wq .spinner-border').remove();
		$('#cb-resubmit-to-exec-wq *[name="Mod0EditRecord"]').val('Submit');
		$('#cb-resubmit-to-exec-wq *[name="Mod0EditRecord"]').attr('onclick', 'resubmitExecWqShowLoading();');

		if($('#cb-resubmit-to-exec-wq *[data-cb-name="HeaderErrorMsg"]').length)
		{
			$('#cb-resubmit-to-exec-wq article').show();
		}
	}

	if((event.detail.appKey).toUpperCase() == ('25d550006acc0c733f0e45b89590').toUpperCase()) // Re-submit to execution - OLD ISA
	{
		$('#cb-resubmit-to-exec-wq .spinner-border').remove();
		$('#cb-resubmit-to-exec-wq *[name="Submit"]').val('Submit');
		$('#cb-resubmit-to-exec-wq *[name="Submit"]').attr('onclick', 'resubmitExecWqShowLoading();');
	...
	$('.cb-deny-isa-cn *[name="Submit"]').attr('onclick', 'denyPopupLoading();');
});

function resubmitExecWqShowLoading()
{
	$('#cb-resubmit-to-exec-wq article').hide();
	$('#cb-resubmit-to-exec-wq').append('<div class="spinner-border text-info" role="status"></div>');
}

function denyPopupLoading()
{
	$('.cb-deny-isa-cn article').hide();
	$('.cb-deny-isa-cn').append('<div class="spinner-border text-info" role="status"></div>');
}

function price_example_popup(price_ex, manufacturer, product_line, invoice_discount, price_level)
{
	var url_param = '';
	...
	if(product_line != '-All-')
	{
		url_param += '&Product_Line=' + encodeURIComponent(product_line);
	}
	
	url_param += '&Invoice_Discount=' + encodeURIComponent(invoice_discount);
	...
	$.confirm({
			...
			title : price_ex,
			theme : 'material',
			content : '<iframe src="./allied-set-up-price-ex.php?' + url_param + '" onload="iframe_autoresize(this)"></iframe>',
...
var findButtons = setInterval(function () {
	...
	if ($(".cb-hotbuttons :button").length >= 2) {
		$(".cb-hotbuttons :button:not(.cb-app-prod-line-btn):not(.cb-error-report-btn)").click(function () { // Exclude Buttons: Error Report, Approved Product Lines - Jun
...
var findCancelButton = setInterval(function () {
	if ($('.jconfirm-closeIcon').length)
	{
		$('.jconfirm-closeIcon').click(function () {
			$('.cb-hotbuttons button').prop('disabled', false).removeClass('disabled');
		});
	}
}, 800);
...
function resubmitToExecutionWQ(isaId, hasRecordInDealExecution)
{
	...
	resubmitToExecutionPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-resubmit-to-exec-wq', 'https://' + accountId + '/dp/' + appKeyPrefix + '596f88aa63cf4078b538/emb?ISA_ID=' + isaId);
		}
	});
}
...
function resubmitToExecutionWQOldIsa(isaId)
{
	resubmitToExecutionPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-resubmit-to-exec-wq', 'https://' + accountId + '/dp/' + appKeyPrefix + '6acc0c733f0e45b89590/emb?ISA_ID=' + isaId);
		}
	});
}
...
function denyByCEO(isaId, lastApproverRoleId)
{
	...
	denyByCEOPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-deny-by-ceo', 'https://' + accountId + '/dp/' + appKeyPrefix + 'e9af141cfe924e169925/emb?ISA_ID=' + isaId + '&Last_Role_ID=' + lastApproverRoleId + '&Status_ID=' + statusId);
		}
	});
}
...
function denyByKN(isaId, lastApproverRoleId)
{
	...
	denyByKNPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-deny-by-kn', 'https://' + accountId + '/dp/' + appKeyPrefix + 'a271fc321d49401286ea/emb?ISA_ID=' + isaId + '&Last_Role_ID=' + lastApproverRoleId + '&Status_ID=' + statusId);
		}
	});
}
...
function denyByCOO(isaId, lastApproverRoleId)
{
	...
	denyByCOOPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-deny-by-coo', 'https://' + accountId + '/dp/' + appKeyPrefix + '0ab81875689b44458f50/emb?ISA_ID=' + isaId + '&Last_Role_ID=' + lastApproverRoleId + '&Status_ID=' + statusId);
		}
	});
}
...
function denyByCFO(isaId, lastApproverRoleId)
{
	...
	denyByCOOPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-deny-by-cfo', 'https://' + accountId + '/dp/' + appKeyPrefix + 'd93ae631b90443ae8880/emb?ISA_ID=' + isaId + '&Last_Role_ID=' + lastApproverRoleId + '&Status_ID=' + statusId);
		}
	});
}
...
function denyByRVPO(isaId, lastApproverRoleId)
{
	...
	denyByRVPOPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-deny-by-rvpo', 'https://' + accountId + '/dp/' + appKeyPrefix + '7d8ed459a4a04e4995ee/emb?ISA_ID=' + isaId + '&Last_Role_ID=' + lastApproverRoleId + '&Status_ID=' + statusId);
		}
	});
}
...
function denyByRVPS(isaId, lastApproverRoleId)
{
	...
	denyByRVPSPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-deny-by-rvps', 'https://' + accountId + '/dp/' + appKeyPrefix + '3f2f9b6da3834f518919/emb?ISA_ID=' + isaId + '&Last_Role_ID=' + lastApproverRoleId + '&Status_ID=' + statusId);
		}
	});
}
...
function denyByIsaTeam(isaId, lastApproverRoleId)
{
	...
	denyByIsaTeamPopup = $.confirm({
		...
		, onOpen : function()
		{
			deployDP('cb-deny-by-isateam', 'https://' + accountId + '/dp/' + appKeyPrefix + '7cddacbfeab74528a6f0/emb?ISA_ID=' + isaId + '&Last_Role_ID=' + lastApproverRoleId + '&Status_ID=' + statusId);
		}
	});
}