...
function checkLiquidStData() {
	...
	if (errorsCountRemSt || errorsCountAcctToRec) {
		...
		var $row = $('.cb-liquid-st tr[data-cb-name="data"]')
			.eq(0)
			.clone();
		$row.removeAttr("id");
		...
		$('.cb-liquid-st table[data-cb-name="cbTable"]').append(
			"<tfoot></tfoot>"
		);
		$('.cb-liquid-st table[data-cb-name="cbTable"] tfoot').append(
			$row
		);
		...
		if (errorsCountAcctToRec) {
			$row.children("td")
				.eq(8)
				.html(
					'<span class="cb-required-msg">** REQUIRED **</span>'
				);
		}
...
function checkLiquidEqData() {
	...
	if (errorsCountRemEq || errorsCountPaidToAcct) {
		// clone 1st row to be used as a container for the required msg
		var $row = $('.cb-liquid-eq tr[data-cb-name="data"]')
			.eq(0)
			.clone();
		$row.removeAttr("id");
		$row.removeAttr("data-cb-name");
		...
		$('.cb-liquid-eq table[data-cb-name="cbTable"]').append(
			"<tfoot></tfoot>"
		);
		$('.cb-liquid-eq table[data-cb-name="cbTable"] tfoot').append(
			$row
		);
...
function checkAlliedStData() {
	...
	if (errorsCountRemEq || errorsCountAccountToReceive) {
		// clone 1st row to be used as a container for the required msg
		var $row = $('.cb-allied-st tr[data-cb-name="data"]')
			.eq(0)
			.clone();
		$row.removeAttr("id");
		$row.removeAttr("data-cb-name");
		...
		$('.cb-allied-st table[data-cb-name="cbTable"]').append(
			"<tfoot></tfoot>"
		);
		$('.cb-allied-st table[data-cb-name="cbTable"] tfoot').append(
			$row
		);
...
function checkAlliedEqData() {
	...
	if (errorsCountRemEq || errorsCountPaidToAccount) {
		// clone 1st row to be used as a container for the required msg
		var $row = $('.cb-allied-eq tr[data-cb-name="data"]')
			.eq(0)
			.clone();
		$row.removeAttr("id");
		$row.removeAttr("data-cb-name");
		...
		$('.cb-allied-eq table[data-cb-name="cbTable"]').append(
			"<tfoot></tfoot>"
		);
		$('.cb-allied-eq table[data-cb-name="cbTable"] tfoot').append(
			$row
		);
...
function showFDRAmounts() {
	var fixedPaymentTotal = $('.cb-fdr-amounts *[name="cbParamVirtual1"]')
		...
		.eq(0)
		.html();
	var fixedDollarRebateTotal = $(
		'.cb-fdr-amounts *[name="cbParamVirtual2"]'
	)
		.closest("td")
		.children("span")
		.eq(0)
...
function loadEditableD2pDps(isaId) {
	...
	deployDP(
		"cb-fdr",
		loadCaspioURL("e86b5b38b3cd4e21a25b", `?ISA_ID=${isaId}`)
	);
...
function loadViewonlyD2PDps(isaId) {
	...
	deployDP(
		"cb-fdr",
		loadCaspioURL("ac75604b79ec49e090c8", `?ISA_ID=${isaId}`)
	);
...
$(document).on(
	...
	function () {
		...
		for (var key in window.dataPageManagerObj.dataPages) {
			if (
				key.search(
					`${appKeyPrefix}696266a218044c13b86a`
				) != -1
			) {
				window.dataPageManagerObj.dataPages[
					key
				].refresh();
			}
		}
	...
	if (appKey.includes("e86b5b38b3cd4e21a25b")) {
		...
		if (typeof window.cbAjaxEventHandler !== "undefined") {
			window.cbAjaxEventHandler.addEventListener(
				...
				function (v_event) {
					$(
						".cb-fdr .cbGridCtnr .BodyCtnr > table"
					).append(
						' \
          <tfoot> \
            <tr> \
        ...
        '
					...
					if (
						appkeysHiddenCheckboxesOnGrid.indexOf(
							v_event.data.AppKey
						) >= 0
					) {
						let form = $(
						...
						if (
							form.find(
								'tr[data-cb-name="data"]'
							).length == 0
						) {
							form.find(
								'*[data-cb-name="BulkEditButton"]'
							).hide();
						}
					}
				}
