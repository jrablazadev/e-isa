...
var internalUsageCb, editedInternalUsage;
...
document.addEventListener("DataPageReady", function (e) {
	if (e.detail.appKey.includes("5a1f6380977a4144b939")) {
		internalUsageCb = document.querySelector(
			'input[name="InsertRecordInternal_Usage"]'
		);
		...
		if (params.has("Opportunity_ID")) {
			...
			document.addEventListener(
				...
				function (e) {
					if (
						...
						var button =
							document.querySelector(
								'button[data-target="#cb-modal-related-customer"]'
							);
						var tr =
							button.closest(
								"tr.cbFormTableRow"
						...
						setTimeout(() => {
							...
							fieldPaint_Manufacturer.value =
								HL_Paint_Manufacturer;
							fieldPaint_Manufacturer.dispatchEvent(
								event
							);
						}, 1500);
						...
						//apply css
						setTimeout(() => {
							fieldCredit_Limit.parentElement.parentElement.style.cssText =
								"display: none;";
						}, 1500);
						fieldReason_For_New_ISA.style.pointerEvents =
							"none";
					}
					...
					internalUsageCb.checked =
						editedInternalUsage != undefined
							? editedInternalUsage
							: "";

					internalUsageCb.addEventListener(
						"click",
						function (e) {
							editedInternalUsage =
								this.checked;
		...
		//.end of New Business
			...
			setTimeout(() => {
				...
				var editedCreditStatus = String(
					hitlistValues["Credit_Status"]
				).replace(/,/g, "")
					? String(
							hitlistValues[
								"Credit_Status"
				...
				var editedPaintManufacturer = String(
					hitlistValues["Paint_Manufacturer"]
				).replace(/,/g, "")
					? String(
							hitlistValues[
								"Paint_Manufacturer"
					...
					: "";
				var editedMPOBased = String(
					hitlistValues["MPO_Based"]
				).replace(/,/g, "")
					? String(
							hitlistValues[
								"MPO_Based"
			...
			fieldsToBePopulated.forEach((fieldName) => {
				var editedField = document.querySelector(
					`[name="InsertRecord${fieldName}"]`
				);
				if (editedField) {
					editedField.addEventListener(
