...
var cogLinks = {
	...
	"1b3d5617e58449fca041": [
		...
		"Manufacturer Subscription Fee Summary",
		"Log1c Subscription Fee Summary",
	], // Deal Directory 2 - CSM

	// RSM
	"976d73e526d3456baef1": [
		"View Details",
		"E-ISA Logs",
		"ComCept AR",
		"ComCept Pricing",
	...
	"63f217cac0c847ad8a1b": [
		...
		"Sales by Product Line",
		"Manufacturer Subscription Fee Summary",
		"Log1c Subscription Fee Summarys",
	], // Deal Directory
	"7229fa8e5f50432da038": [
		"View Details",
		"Edit Details",
		"Comments",
		"Attachments",
	...
	"7fb89b3497914e4c9e47": [
		...
		"Manufacturer Subscription Fee Summary",
		"Log1c Subscription Fee Summary",
	], // Deal Directory 2 - CEO, CFO, COO, KN
	"6692510839ef4e1e90cc": [
		"Log AMP - ISA ID",
		"View Details",
		"Comments",
		"Attachments",
		...
		"Manufacturer Subscription Fee Summary",
		"Log1c Subscription Fee Summary",
	], // Related Accounts Search
	"71f1f8e38658430eb8ec": [
		"View Details",
		"Attachments - Filtered",
		"Comments",
		"Related Accounts - View",
	...
	cf23ebc6ba694402b926: [
		...
		"Manufacturer Subscription Fee Summary",
		"Log1c Subscription Fee Summary",
	], // Related Accounts Search All E-ISA

	// Users
	"9813a8444b764f8590ac": [
		"Log AMP - ISA ID",
		"View Details",
		"Comments",
		"Attachments",
...
$(document).on("click", ".cb-sales-by-prod-tsm-btn", function (e) {
	...
	var isaName = $(this).attr("data-isa-name");
	window.open(
		`https://${accountId}/dp/${appKeyPrefix}f41477fde71c4f8f824b?ISA_ID=${isaId}`
	);
	e.preventDefault();
});
...
$(document).on("click", ".cb-sales-by-prod-btn", function (e) {
	...
	var isaName = $(this).attr("data-isa-name");
	window.open(
		`https://${accountId}/dp/${appKeyPrefix}6be4e2361e184c20ac66?ISA_ID=${isaId}`
	);
	e.preventDefault();
});
...
$(document).on("click", ".cb-log-amp-btn", function (e) {
	...
	window.open(
		`https://${accountId}/dp/${appKeyPrefix}085a1a0e10d040a4a304?cid=${customerId}`
	);
	e.preventDefault();
});
...
$(document).on("click", ".cb-log-amp-isa-id-btn", function (e) {
	...
	window.open(
		`https://${accountId}/dp/${appKeyPrefix}8da940d4399e4b979ab0?ISA_ID=${isaId}`
	);
	e.preventDefault();
});
...
$(document).on("click", ".rebate-details-btn", function (e) {
	...
	window.open(
		`https://${accountId}/dp/${appKeyPrefix}88121621466943ffa380?ID=${isaId}`
	);
	e.preventDefault();
});
...
$(document).on("click", ".mfr-subs-fee-summary-btn", function (e) {
	...
	jc_open_iframe(
		`<div class="mb-2">ISA ${isa_id}</div><div>Manufacturer Subscription Fee Summary</div>`,
		`../shared/mfr-fee-summary.php?ISA_ID=${isa_id}`
	);
	e.preventDefault();
});
...
$(document).on("click", ".log1c-subs-fee-summary-btn", function (e) {
	...
	jc_open_iframe(
		`<div class="mb-2">ISA ${isa_id}</div><div>Log1c Subscription Fee Summary</div>`,
		`../shared/log1c-fee-summary.php?ISA_ID=${isa_id}`
	);
	e.preventDefault();
});
...
var timer = setInterval(function () {
	$(".cb-report, .cb-results").each(function () {
		...
		if (
			$(this).find(".dropdown-menu.show").length != 0 &&
			$(this).attr("data-height") != "Y"
		) {
			$(this).css({
				height:
...
function create_cog(elem) {
	...
	const subfolder = window.parent.location.pathname
		.split("/")
		.at(-2)
		.toLowerCase();
	const arr_removed_renewDeal = [
		"so",
		"ar",
		"srf",
		"ap",
	...
	cogHTML +=
		'<div class="dropdown dropright" style="z-index:9999999 !important;" data-height="N">';
	cogHTML +=
		'<a class="btn btn-link btn-sm dropdown-toggle" href="javascript:void(0)" role="button" data-toggle="dropdown" aria-haspopup="true" data-bs-display="static" aria-expanded="false" title="';
	...
	if (arr_removed_renewDeal.includes(subfolder)) {
		cogLinks[appKey] = cogLinks[appKey].filter(
			(item) => item !== "Create New Deal"
		);
	}
